...
contract NNNToken is ERC20PresetMinterPauser("NNN Token", "NNN") {
    ...
    constructor(uint32 _tokenTransferFee, address _feeAddress) public {
        require(
            _tokenTransferFee > 0,
            "NNN Token contract: fee must be bigger than 0"
        );
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if (w.isWhitelisted(address(this)) == false) {
    ...
    function transferFrom(
        ...
        if (w.isWhitelisted(sender) == false) {
            // calculate transfer fee and send to predefined wallet
            uint256 transferFeeAmount = amount.div(tokenTransferFeeDivisor);
            ...
            return
                super.transferFrom(
                    sender,
                    recipient,
                    amount.sub(transferFeeAmount)
                );
        } else return super.transferFrom(sender, recipient, amount);
    }

    function mintWithoutDecimals(address recipient, uint256 amount)
        public
    {
        return super.mint(recipient, amount * 1000000000000000000);
    }
