...
contract('NNNToken (proxy)', async accounts => {
  before(async function () {
    ...
    this.nnnToken.setTransferFeeDivisor(my_constants._t_c.FEE);
  });
  ...
  it('has 18 decimals', async function () {
    expect(await this.nnnToken.decimals()).to.be.bignumber.equal('18');
  });

  it("transfer fee should be 1/" + my_constants._t_c.FEE, async function () {
  ...
  it("transfer address should be " + accounts[1], async function () {
    let feeAddress = await this.nnnToken.feeAddress();
    assert.equal(feeAddress.toString(), accounts[1]);
  });

  it("exclude role should be " + my_constants._t_c.FEE_EXCLUDED_ROLE, async function () {
